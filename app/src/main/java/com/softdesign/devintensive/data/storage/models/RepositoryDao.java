...
public class RepositoryDao extends AbstractDao<Repository, Long> {
    ...
    */
    ...
    private Query<Repository> userDb_RepositoriesQuery;
    ...
    /** Internal query to resolve the "repositories" to-many relationship of UserDb. */
    public List<Repository> _queryUserDb_Repositories(String userRemoteId) {
        synchronized (this) {
            if (userDb_RepositoriesQuery == null) {
                QueryBuilder<Repository> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.UserRemoteId.eq(null));
                userDb_RepositoriesQuery = queryBuilder.build();
            }
        }
        Query<Repository> query = userDb_RepositoriesQuery.forCurrentThread();
